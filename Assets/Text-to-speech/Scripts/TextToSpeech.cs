...
namespace GoogleTextToSpeech.Scripts
{
    public class TextToSpeech : MonoBehaviour
    {
        ...
        public void GetSpeechAudioFromGoogle(
            string textToConvert,
            VoiceScriptableObject voice,
            Action<AudioClip> audioClipReceived,
            Action<BadRequestData> errorReceived
        )
        {
            _actionRequestReceived = null;
            _actionRequestReceived += (
                requestData => RequestReceived(requestData, audioClipReceived)
            );
            ...
            RequestService.SendDataToGoogle(
                ...
                dataToSend, apiKey, _actionRequestReceived, errorReceived
            );
        }

        private static void RequestReceived(
            string requestData, Action<AudioClip> audioClipReceived
        )
        {
